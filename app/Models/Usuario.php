...
class Usuario extends Model {
    ...
    public static function buscaCargo ( int $idUsuario ) : array {
        ...
        $stmt->execute([ 'usuid' => $idUsuario ]);
        return $stmt->fetchAll(PDO::FETCH_COLUMN);
    }

    public static function buscaPermissao ( int $idUsuario ) : array {
        $stmt = parent::pegarBanco()->prepare( ' SELECT permissoes.nome FROM permissoes INNER JOIN cargo_permissao ON cargo_permissao.permissao_id = permissoes.id INNER JOIN usuario_cargo ON usuario_cargo.cargo_id = cargo_permissao.cargo_id WHERE usuario_cargo.usuario_id = :usid ORDER BY permissoes.nome ');
